...
Создает файл `.python-version` в проекте, тем самым определив версию питон для конкретного проекта

    $ uv python pin 3.11

Специально устанавливать питон для проекта не нужно, uv установит его автоматически, при первом запуске питон (естественно, если будет выполнено что то вроде `uv run python -V`, а не просто `python -V`).
...
Но для этого в pyproject.toml должно быть "разрешено" устанавливать более высокие версии. Например, если указано "django==4.1.7", то пакет не обновится. Нужно указать "django>=4.1.7". При этом надо понимать, что сам пакет обновиться физически, и это синхронизируется с uv.lock. Но pyproject.toml автоматом не обновиться. 
